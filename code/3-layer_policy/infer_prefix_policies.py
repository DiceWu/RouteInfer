...
def emulate(prefix,desas,tempqr,initroute,initqueue,initcache,deb=None,tick1=0):
    route = initroute
    queue = initqueue
    if len(queue) == 0:
        for nei in link[desas]:
...
for prefix in prefixes:
    ...
    while True:
        ...
        initroute = route
        initcache = cache
        route,cache = emulate(prefix,desas,tempqr,initroute,initqueue,initcache,tick1=tick1)
        if route == False:
            break
    del paths[prefix]
